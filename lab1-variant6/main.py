...
Параметры варианта 6:
  ...
  x0 = a/2 (для корней)
  Доп. требование Д1: таблицы итераций
"""
...
def sqrt_newton(a, eps=EPS, n_max=N_MAX):
    ...
    Начальное приближение: x0 = a/2 (согласно варианту 6).
    Критерий остановки: |x_new - x| < eps.
    """
    if a < 0:
        raise ValueError(
...
def cbrt_newton(b, eps=EPS, n_max=N_MAX):
    ...
    Начальное приближение: x0 = |b|/2 (согласно варианту 6).
    Учитывается знак аргумента.
    """
    if b == 0:
        return 0.0, 0
...
def print_iteration_table_sqrt(a, eps=EPS):
    """Печать таблицы итераций для квадратного корня (Д1)."""
    x = a / 2.0  # x0 = a/2
    print("\nИтерации для sqrt(a):")
    print(f"{'n':>3} {'x_n':>18} {'|x_(n+1) - x_n|':>20}")
    print("-" * 45)
...
def print_iteration_table_cbrt(b, eps=EPS):
    """Печать таблицы итераций для кубического корня (Д1)."""
    sign = -1 if b < 0 else 1
    b_abs = abs(b)
    x = b_abs / 2.0  # x0 = |b|/2
...
def print_iteration_table_ln(c, eps=EPS):
    """Печать таблицы итераций для натурального логарифма (Д1)."""
    y = (c - 1) / (c + 1)
    y2 = y * y
    power = y
    total = 0.0
    ...
    print("\nИтерации для ln(c):")
    print(f"{'k':>3} {'S (сумма)':>18} {'|t_k|':>20}")
    print("-" * 45)

    for k in range(10000):
        term = 2 * power / (2 * k + 1)
        print(f"{k:>3} {total:>18.12f} {abs(term):>20.12e}")
...
def main():
    ...
    cbrt_reference = -math.pow(27, 1 / 3)  # корректный способ для отрицательного
    ...
    print("=" * 95)
    print("ЛАБОРАТОРНАЯ РАБОТА № 1 — ВАРИАНТ 6")
    ...
    print("Критерий остановки для корней: по разности соседних приближений")
    print("Начальное приближение: x0 = a/2")

    # Требование Д1: таблицы итераций
    print_iteration_table_sqrt(a, eps)
    print_iteration_table_cbrt(b, eps)
    print_iteration_table_ln(c, eps)
